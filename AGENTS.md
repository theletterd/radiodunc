...
- **`ANALYSER_FFT_SIZE` is coupled to `ANALYSER_BANDS`** — changing one without rechecking the other is the trap here. A band narrower than one FFT bin can't be measured independently, so neighbouring bars read the same bins and move as one clump. At 34 bands with a 2048-point FFT, measured neighbour correlation across the bottom bars was **0.93** — essentially the same bar drawn seven times — against 0.71 at 8192. At the current 20 bands every band is comfortably wider than an 11.7 Hz bin, so 4096 suffices, and it's the better choice anyway: an 85 ms analysis window against 8192's 170 ms, i.e. visibly snappier on transients. Going back above ~20 bands means returning to 8192. A test pins both values so this can't drift silently.
...
- **Segmented LEDs.** Bars are a stack of discrete lit/unlit blocks (`ANALYSER_SEG_H` / `ANALYSER_SEG_GAP`), not a solid fill. Unlit segments are painted too, very dim (`ANALYSER_UNLIT_COLOUR`) — that faint always-visible grid is most of what sells the old-display look, along with the recessed-panel background on `.analyser` in styles.css.
- **Level zones.** `analyserZoneColour(fraction, lowColour)` (pure, unit-tested) escalates by height the way hardware does: the station's on-air colour holds the low zone below `ANALYSER_ZONE_MID`, then amber, then red above `ANALYSER_ZONE_HOT`. Low-zone colour comes from `ANALYSER_LOW_COLOUR` keyed by `onAirMode`, mirroring the `#nowPlaying[data-mode=...]` colours in styles.css (pink music/DJ, orange ads, blue news). The hot zones are shared across modes — red means *loud*, not *which segment is on air*.
- **Peak-hold markers.** Per band, the marker jumps instantly to a new high, holds `ANALYSER_PEAK_HOLD_MS`, then sinks at `ANALYSER_PEAK_FALL_PER_S`. The hold is load-bearing: without it the marker just trails the bar down and reads as lag rather than an indicator. Both are **wall-clock, not frame counts** — frame-counting silently halved the durations on any 120 Hz ProMotion display, which is what this runs on. `_drawAnalyserFrame(dtMs)` takes elapsed time as an argument (defaulting to a nominal frame so tests stay deterministic) and the rAF loop feeds it real timestamps. A frame straddling the hold/fall boundary splits its time between the two, so total duration doesn't drift with frame rate; there's a test asserting the same elapsed time decays identically across 7 frames and 84.
- **Glow.** Lit cells are drawn twice — a blurred, semi-transparent halo pass via `ctx.filter`, then sharp on top — so each cell's halo takes its own colour, which a single CSS drop-shadow couldn't do. One batched GPU blur is far cheaper than per-rect `shadowBlur` across ~600 cells, and where `ctx.filter` is unsupported the pass is skipped and the display just renders without bloom. `ANALYSER_GLOW_ALPHA` is tuned to keep lit rows ~3.5× brighter than the gaps between them, so the bloom softens the grid without closing it. It's painted near-white rather than in the zone colour (a red marker would vanish against the red hot zone) and only drawn once it has *detached* from the bar — while the signal is still climbing, the marker sits inside the lit stack and painting it white there would bleach the top of every bar.
